...
cron() {
    ...
    while true; do
        to=()                                                            # 存放本次需要更新的模块
        [ $((i % 60)) -eq 0 ]  && to=(${to[@]} wifi)                     # 每 10秒  更新 wifi
        [ $((i % 10)) -eq 0 ]  && to=(${to[@]} cpu mem vol icons)        # 每 20秒  更新 cpu mem vol icons
        [ $((i % 10)) -eq 0 ] && to=(${to[@]} bat)                      # 每 300秒 更新 bat
        [ $((i % 20)) -eq 0 ]   && to=(${to[@]} date)                     # 每 5秒   更新 date
